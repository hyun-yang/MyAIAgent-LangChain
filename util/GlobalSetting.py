...
class GlobalSetting(QDialog):
    def __init__(self):
        ...
        self.create_langchain_group(row1Layout)
    ...
    def create_langchain_group(self, layout):
        langchainGroup = QGroupBox('Langchain / Tavily')
        langchainLayout = QGridLayout()
        self.langchain_labels = ['Langchain API', 'Langchain Endpoint', 'Langchain tracing v2',
                                           'Langchain Project', 'Tavily API']
        self.langchain_keys = ['langchain_api_key', 'langchain_endpoint', 'langchain_tracing_v2',
                                         'langchain_project', 'tavily_api_key']
        self.langchain_editors = [QLineEdit() for _ in self.langchain_labels]

        for i in range(len(self.langchain_editors)):
            self.langchain_editors[i].textChanged.connect(partial(self.handle_langchain_value_change, i))

        for i, label in enumerate(self.langchain_labels):
            langchainLayout.addWidget(QLabel(label), i, 0)
            langchainLayout.addWidget(self.langchain_editors[i], i, 1, 1, 2)

            langchain_value = self._settings.value(f'Langchain/{self.langchain_keys[i]}')
            if langchain_value:
                self.langchain_editors[i].setText(langchain_value)

        langchainGroup.setLayout(langchainLayout)
        layout.addWidget(langchainGroup)
    ...
    def handle_langchain_value_change(self, index, text):
        self._settings.setValue(f'Langchain/{self.langchain_keys[index]}', text)
